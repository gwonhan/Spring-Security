    ...
    *  정적인 리소스에 대한 요청을 제외하는 설정을 하는 Bean*/
    ...
    public SecurityFilterChain configure(HttpSecurity http) throws Exception {
        ...
        http.authorizeHttpRequests(auth->{
            ...
            login.failureHandler(authFailHandler);             // 로그인에 실패했을시 처리할 커스텀 핸들러
            ...
            session.maximumSessions(1); // session 의 허용갯수 제한 -> 한 사용자가 여러창을 띄어 동시에 여러개 세션 활성화 방지
            session.invalidSessionUrl("/"); // 세션이 만료되었을 때 이동할 페이지
        }).rememberMe(remember->{
            remember.tokenValiditySeconds(604800); // 토큰의 유효기간 설정
            remember.key("mykey"); // remember - me 토큰을 생성하는데 사용되는 키
        }).csrf( csrf->csrf.disable()); // 추가적인 구현이 필요하기 떄문에 비활성화
